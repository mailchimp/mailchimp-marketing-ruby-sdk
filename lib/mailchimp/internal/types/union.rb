    ...
    module Types
      ...
      module Union
        ...
        private def type_matches?(value, member_type)
          case member_type
          when Mailchimp::Internal::Types::Hash
            value.is_a?(::Hash)
          when Mailchimp::Internal::Types::Array
            value.is_a?(::Array)
          when Class, Module
              ...
              members.find do |_key, mem|
                ...
                begin
                  ...
                  member_type.fields.each do |field_name, field|
                    raise Errors::TypeError, "Required field `#{field_name}` missing for union member #{member_type.name}" if candidate.instance_variable_get(:@data)[field_name].nil? && !field.optional && !(field.nullable && [field.api_name, field.name].any? { |k| value.key?(k.to_sym) || value.key?(k.to_s) })
                  end
